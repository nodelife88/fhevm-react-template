...
export function FhevmDemo() {
  ...
  const ethersSigner = useMemo(() => {
    if (!isConnected || !window.ethereum) return undefined
  ...
  const { encryptWith } = useFHEEncryption({ 
    instance: fhevmInstance, 
    ethersSigner,
    contractAddress: MESSENGER_ADDRESS
  })

  // Decryption requests
  const requests = useMemo(() => {
    if (!encryptedMessage) return undefined
    return [{ handle: encryptedMessage, contractAddress: MESSENGER_ADDRESS }] as const
  }, [encryptedMessage])
  ...
  if (!isConnected) {
    return (
      <Card className="w-full max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle>FHEVM Integration</CardTitle>
          <CardDescription>Connect your wallet to use FHEVM encryption/decryption</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Alert>
            <AlertDescription>
              Please connect your wallet to use FHEVM encryption/decryption.
            </AlertDescription>
          </Alert>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>FHEVM SDK Integration</CardTitle>
          <CardDescription>
            Real FHEVM encryption and decryption with ConfidentialMessenger contract
          </CardDescription>
          <div className="flex gap-2">
            ...
            <Badge variant="default">Contract: 0x663F72147269D638ED869f05C0B4C62008826a6b</Badge>
          </div>
        </CardHeader>
      </Card>
